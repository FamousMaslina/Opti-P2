...
try:
    ...
    print("Warning - requests module not installed.")
    requestsM = False
try:
    import platform
except ImportError:
    print("Warning - Platform module not installed.")
    platformM = False
modules = [
...
import configparser
def get_missing_libs():
    print("\nIf True, it means that it exists.")
    print("False means that it doesn't exist.")
    print("\nrequests", requestsM)
    print("platform", platformM,"\n")
...
def root():
    ...
    global current_directory
    current_directory = initial_directory
    #print(f"Returned to root directory")
...
def mainOS():
    ...
    command_ram_usage = {
        ...
        'computername': 15, 'manage_extensions': 50, 'ram': 5, 'mkdir': 10, 'cd': 5, 'ls': 5, 'touch': 10, 'root': 1, 'get_missing_libs': 2
    }
    ...
    available_memory = mb_module.mMem 
    if platform == False:
        print("Extra module not installed! 'platform'", platform)
        pass
    if requests == False:
        print("Extra module not installed! 'requests'", requests)
        pass
    ...
    while True:
        ...
        if relative_path == ".":
            relative_path = "root"
        if syst['users'] == "1":
            inp = input(f"O:/{current_user}/{relative_path}/> ").strip().lower()
        else:
            inp = input(f"O:/{relative_path}/> ").strip().lower()
        ...
        if inp in default_commands:
            ...
            if available_memory >= ram_required:
                try:
                    ...
                    if inp == 'mkdir':
                        ...
                        folder_name = input("Enter folder to navigate to: ").strip()
                        cd(folder_name)
                    elif inp == 'ls':
                        ls()
                    elif inp == 'touch':
           ...
           #print("files", files)
            ...
            if intern == 1 and requests == True:
                ...
                print()
            elif intern == 0 and requests == False:
                print()
                print("Not Connected to internet and 'requests' module not installed")
                print()
            elif requests == False:
                print("\n'request' module not installed!\n")
