...
def test():
    clear()
    file_path = 'op2.ini'  # Replace with your config file path
...
def credits():
    if settings['computer_name'] == "DEBUG_MODE":
        ...
        print("Thus,",op2v.op2VER,"will be the last update.\n")
...
def mainOS():
    ...
    while True:
        ...
        inp = inp.lower()
        if inp in ('bios', 'info', 'cls', 'exit', 'help', 'gpu', 'restart', 'gpuinfo', 'modem', 'internet', 'api', 'encryp', 'nguess', 'write', 'calc', 'resethardware', 'hardware', 'users', 'virtualcommand', 'omclient', 'omserver', 'upkeep','remids', 'date', 'hour','gen', 'gensys', 'reboot', 'shutdown', 'configuration','test', 'credits', 'computername'):
            eval(inp)()
